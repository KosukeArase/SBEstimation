	   ...
	   int maxevals, int inseed, double* output, int printToFile, int sample_symmetry, MPI_Comm spawn_comm, int num_of_spawn_comm)
{
    ...
    scatter_sendvec = (double *)malloc(sizeof(double) * N * (lambda + num_of_pop_per_spawn));
    scatter_rcvvec = (double *)malloc(sizeof(double) * num_of_pop_per_spawn * N);
...
#pragma omp parallel for reduction(+:mueff)
    ...
    double t_neuron_start, t_neuron_end;
	...
	for(i=0; i<lambda; ++i){
	    ...
	    for(k=0;k<N;++k){
	      scatter_sendvec[i*N+k+(num_of_pop_per_spawn * N)] = x_temp[k];
	      printf("scatter_send_vec_parent[%d][%d] = %lf\t",i, k, scatter_sendvec[i*N+k+(num_of_pop_per_spawn * N)]);
	    }
	    printf("\n");
	    if (sample_symmetry)
		sign = -sign;
	...
	t_neuron_start = MPI_Wtime();
	//scatter gene information from parent to child, and from child to grandchild	    
	MPI_Scatter(scatter_sendvec, num_of_pop_per_spawn * N, MPI_DOUBLE, scatter_rcvvec, num_of_pop_per_spawn * N, MPI_DOUBLE, 0, spawn_comm);
	//MPI_Scatter(,,,nrn_comm); //in make_neuro_spawn and NEURON
	t_end_util = MPI_Wtime();
	
	printf("elapse time for MPI_Scatter from parent to child is %lf\n", t_end_util - t_neuron_start);
	...
	t_neuron_end = MPI_Wtime();

	printf("elapse time for MPI_Gather from child to parent is %lf\n", t_neuron_end - t_start_util);
	printf("elapse time from passing genes to recieve NEURON results is %lf(sec)\n", t_neuron_end - t_neuron_start);
	...
	if (arfitness[0] < target_f){
	  printf("stop: arfitness is sufficiently small\n");
	    stop = 1;
	}
	if (counteval >= maxevals){
	  printf("stop: loop more than maxevals\n");
	  printf("counteval = %d, maxevals = %d\n", counteval, maxevals);
	  stop = 1;
	}
	itr = itr + 1;
	for(i=0;i<N;++i)	
	  if (sigma[i] < 1e-20){
	    if(counteval < (maxevals * 0.75)){
	      sigma[i] = 0.5 * (xmax[i] - xmin[i]);
	    }else{
	      printf("Stop: sigma becomes small\n");
	      stop = 1;
	    }
	  }
	...
	t_end = MPI_Wtime();
	if(loop_count%50==1)
	  printf("elapse time for MPI_Bcast in parent is %lf\n", t_end - t_start_util);
	printf("elapse time for one loop is %lf(sec)\n stop = %d %dth loop\n", t_end - t_start, stop, loop_count);
    }
    
    printf("loop end #fbest: %lf\n", BestF);
    my_boundary_transformation(&my_boundaries, xbestever, x_temp, 0);
    printGene(stdout, x_temp, N); 
    printf("\n");
    fflush(stdout);
    ...
    free(scatter_rcvvec); free(scatter_sendvec);
}
...
int loadRangeFile(char *filename, double *xmin_vec, double *xmax_vec, int *N){
	...
	//printf("%dth reading in loadRangeFile\n", dimension);
...
int main(int argc, char **argv){
    ...
    char range_filename[] = "./params_onlyWeight.txt";

    double t_start, t_end;
    double t_start_all, t_end_that_time;
    
    MPI_Init(&argc, &argv);
    printf("end of MPI_Init\n");
    MPI_Comm_size(MPI_COMM_WORLD, &main_size);
    printf("end of MPI_Comm_size\n");
    MPI_Comm_rank(MPI_COMM_WORLD, &main_myid);
    printf("end of MPI_Comm_rank\n");
    ...
    t_end_that_time = MPI_Wtime();
    printf("elapse time till start of loadRangeFile is %lf(sec)\n", t_end_that_time - t_start_all);
    printf("start loadRangeFile\n");
    loadRangeFile(range_filename, xmin_tmp, xmax_tmp, &N);
    printf("end loadRangeFile\n");
    t_end_that_time = MPI_Wtime();
    printf("elapse time till end of loadRangeFile is %lf(sec)\n", t_end_that_time - t_start_all);
    ...
    t_end_that_time = MPI_Wtime();
    printf("elapse time till argv->lambda etc is %lf(sec)\n", t_end_that_time - t_start_all);
    ...
    t_end_that_time = MPI_Wtime();
    printf("elapse time till sprintf is %lf(sec)\n", t_end_that_time - t_start_all);
    ...
    t_end = MPI_Wtime();
    printf("elapse time for MPI_Comm_spawn is %lf(sec)\n", t_end - t_start);
    fflush(stdout);
    printf("start LMCMA\n");
