...
extern int nrn_get_mechtype();
 ...
 "setdata_ka_ib", _hoc_setdata,
 "settables_ka_ib", _hoc_settables,
 0, 0
};
...
#define usetable usetable_ka_ib
 double usetable = 1;
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 ka_ib /volume1/home/hp120263/k01792/workspace/github/sb_estimation/hocfile_forSB/sparc64/ka_ib.mod\n");
...
static int _reset;
static char *modelname = "Potasium Type A current for RD Traub et al 2005";
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   minf = 1.0 / ( 1.0 + exp ( ( - _lv - 60.0 ) / 8.5 ) ) ;
   mtau = 0.185 + 0.5 / ( exp ( ( _lv + 35.8 ) / 19.7 ) + exp ( ( - _lv - 79.7 ) / 12.7 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 78.0 ) / 6.0 ) ) ;
   if ( _lv <= - 63.0 ) {
     htau = 0.5 / ( exp ( ( _lv + 46.0 ) / 5.0 ) + exp ( ( - _lv - 238.0 ) / 37.5 ) ) ;
     }
   else {
     htau = 9.5 ;
     }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ...
   alphah = hinf / htau ;
   betah = 1.0 / htau - alphah ;
   alpham = minf / mtau ;
   betam = 1.0 / mtau - alpham ;
   }
 _current += ik;
