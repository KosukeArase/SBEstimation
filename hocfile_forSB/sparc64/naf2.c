	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 naf2 /volume1/home/hp120263/k01792/workspace/github/sb_estimation/hocfile_forSB/sparc64/naf2.mod\n");
...
static int _reset;
static char *modelname = "Second Sodium transient current for Traub RD et al 2005";
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv1 ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv1 ;
 {
   ...
   if ( ( _lv1 + fastNa_shift ) < - 30.0 ) {
     mtau = 0.0125 + 0.1525 * exp ( ( ( _lv1 + fastNa_shift ) + 30.0 ) / 10.0 ) ;
     }
   else {
     mtau = 0.02 + a + ( 0.145 + b ) * exp ( ( - ( _lv1 + fastNa_shift + d ) - 30.0 ) / ( 10.0 + c ) ) ;
     }
   hinf = 1.0 / ( 1.0 + exp ( ( ( _lv1 + fastNa_shift * 0.0 ) + 58.3 ) / 6.7 ) ) ;
   htau = 0.225 + 1.125 / ( 1.0 + exp ( ( ( _lv1 + fastNa_shift * 0.0 ) + 37.0 ) / 15.0 ) ) ;
    return 0; }
