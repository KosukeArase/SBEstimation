...
extern int nrn_get_mechtype();
 ...
 "setdata_cat_a", _hoc_setdata,
 "settables_cat_a", _hoc_settables,
 0, 0
};
...
#define usetable usetable_cat_a
 double usetable = 1;
	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 1);
 	...
 	ivoc_help("help ?1 cat_a /volume1/home/hp120263/k01792/workspace/github/sb_estimation/hocfile_forSB/sparc64/cat_a.mod\n");
...
static int _reset;
static char *modelname = "Calcium low threshold T type current version a for RD Traub 2005";
...
static int  _f_settables ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   minf = 1.0 / ( 1.0 + exp ( ( - _lv - 52.0 ) / 7.4 ) ) ;
   mtau = 1.0 + .33 / ( exp ( ( _lv + 27.0 ) / 10.0 ) + exp ( ( - _lv - 102.0 ) / 15.0 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 80.0 ) / 5.0 ) ) ;
   htau = 28.30 + .33 / ( exp ( ( _lv + 48.0 ) / 4.0 ) + exp ( ( - _lv - 407.0 ) / 50.0 ) ) ;
    return 0; }
